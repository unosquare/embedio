...
namespace EmbedIO.Actions
{
    ...
    public class RedirectModule : WebModuleBase
    {
        ...
        /// <param name="location">The redirect URL.</param>
        ...
        /// <para><paramref name="location"/> is not a valid URL.</para>
        /// <para>- or -</para>
        /// <para><paramref name="statusCode"/> is not a redirection (3xx) status code.</para>
        /// </exception>
        /// <seealso cref="WebModuleBase(string)"/>
        public RedirectModule(string baseRoute, string location, HttpStatusCode statusCode = HttpStatusCode.Found)
            : this(baseRoute, location, null, statusCode, false)
        {
        }
        ...
        /// <para><paramref name="location"/> is <see langword="null"/>.</para>
        /// <para>- or -</para>
        ...
        /// </exception>
        /// <seealso cref="WebModuleBase(string)"/>
        public RedirectModule(string baseRoute, string location, Func<IHttpContext, bool>? shouldRedirect, HttpStatusCode statusCode = HttpStatusCode.Found)
            : this(baseRoute, location, shouldRedirect, statusCode, true)
        {
        }

        private RedirectModule(string baseRoute, string location, Func<IHttpContext, bool>? shouldRedirect, HttpStatusCode statusCode, bool useCallback)
            : base(baseRoute)
        {
            Location = Validate.Url(nameof(location), location);
        ...
        protected override Task OnRequestAsync([ValidatedNotNull] IHttpContext context)
        {
            ...
            {
                context.Redirect(Location, (int)StatusCode);
                context.SetHandled();
            }
