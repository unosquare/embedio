...
namespace Unosquare.Labs.EmbedIO.Tests
{
    ...
    {
        protected List<string> InstancesNames
            => new List<string>() {string.Empty, "A/", "B/", "C/", "A/C", "AAA/A/B/C", "A/B/C/"};

        protected List<StaticFileWebServerTestSubject> Subjects { get; set; } =
            new List<StaticFileWebServerTestSubject>();
        ...
        public void FileContentsMatchInstanceName()
        {
            foreach (var item in Subjects)
            {
                ...
                using (var response = (HttpWebResponse) request.GetResponse())
                {
                    ...
                    Assert.AreEqual(html, TestHelper.GetStaticFolderInstanceIndexFileContents(item.InstanceName),
